...
opp.rank.by.week <- 
  ...
  add_trace(y = ~Ipswich, name = 'Ipswich Town', type = 'scatter', mode = 'lines', 
  ...
  add_annotations(x = 2, y = 6, text = paste('Mean Arsenal Opponent =', round(mean(orbw.data$Arsenal, na.rm = TRUE), 2)),
                  xref = 'x', yref = 'y', showarrow = F, xanchor = 'left') %>%
  add_annotations(x = 2, y = 5, text = paste('Mean Manchester City Opponent =', round(mean(orbw.data$ManCity, na.rm = TRUE), 2)),
                  xref = 'x', yref = 'y', showarrow = F, xanchor = 'left') %>%
