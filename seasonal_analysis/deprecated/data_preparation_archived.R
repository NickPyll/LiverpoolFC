...
# frozen as of June 13, 2023 and no longer updated
...
#   pull()
# 
# # create vector for team linetypes
# teamlinetypes <- 
#   premier.league.clubs |>
#   select(TeamLineType) |>
#   pull()
...
# max(as.Date(as.character(x.seasons$Date), format = '%d/%m/%Y')) # hmmm why is this broken now
...
x.data <-
  ...
  bind_rows(
    x.data |> 
      ...
      distinct() |>
      mutate(Week = 0,
             # update_me Everton point deduction
             # PointsEarned = 0,
             PointsEarned = if_else(Team == 'Everton', -10, 0),
             GoalDifferential = 0,
             GoalsScored = 0,
             GoalsConceded = 0,
             played = 0)
...
# z.rbw.data <- unlist(rbw.data[nrow(rbw.data), -1])
# rbw.data <- rbw.data[, c("Week", names(sort(z.rbw.data, decreasing = FALSE)))]
